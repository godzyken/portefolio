...
class ProjectWizardLeadService {
  ...
  Future<void> submit({
    ...
    if (SupabaseService.isReady) {
      try {
        await SupabaseService.client.from('portfolio_project_leads').insert({
          ...
          'tech_constraints': description.technicalConstraints.isEmpty
              ? null
              : description.technicalConstraints,
          'budget':
              description.budgetRange.isEmpty ? null : description.budgetRange,
        ...
        developer.log('❌ Erreur insertion Supabase: $e',
            name: 'ProjectWizardLeadService', error: e, stackTrace: st);
    ...
    try {
      ...
      buffer.writeln(
          'Contraintes : ${description.technicalConstraints.isEmpty ? "-" : description.technicalConstraints}');
      buffer.writeln(
          'Budget : ${description.budgetRange.isEmpty ? "-" : description.budgetRange}');
      buffer.writeln();
      ...
      if (selectedStrategy != null) {
        ...
        buffer.writeln();
        aiAnalysisText =
            "Stratégie choisie : ${selectedStrategy.title}\n${selectedStrategy.description}";
      } else if (aiSummary != null) {
        buffer.writeln(
            '⚠️ NOTE : Le client n\'a pas sélectionné de stratégie mais l\'IA a analysé le projet.');
        aiAnalysisText = aiSummary;
      } else {
        buffer.writeln(
            'ℹ️ NOTE : Analyse IA non disponible (possible quota épuisé).');
      ...
      developer.log('✅ Email de notification projet envoyé',
          name: 'ProjectWizardLeadService');
    } catch (e, st) {
      developer.log('❌ Erreur envoi EmailJS: $e',
          name: 'ProjectWizardLeadService', error: e, stackTrace: st);
